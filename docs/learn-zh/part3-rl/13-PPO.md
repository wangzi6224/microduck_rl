...
>
> **本章新词**：同策略、重要性采样、比率、裁剪、PPO、代理目标、价值裁剪、KL 散度、自适应学习率。
...
![四步总览：旧策略采数据时存下动作0.5、当时的ln π为−1.0439、优势+2；更新时用现在的网络对同一个动作重算ln π得−0.9639，比率exp(0.08)=1.0833；比率走出0.8到1.2时有利的一边不再加分，比如1.5时取min(3,2.4)=2.4；三项加成损失、KL调学习率，98,304条记录切4份过5遍共更新20次后扔掉](../figures/ch13_overview.png)
...
| 价值裁剪 | critic 的打分，一次最多挪多少？ | 对一道菜的口碑预判，一次也别改太多 | 13.4 |
...
- **三种"裁剪"**：第 3 章 3.7 节的梯度裁剪，剪的是梯度箭头的长度；13.3 节 PPO 的裁剪，剪的是比率——一条记录最多还能为它的动作加多少分；
  13.4 节 critic 那边还有一道价值裁剪，夹的是"这一次迭代里，价值估计挪了多远"，而且两个里取的是**大**的、不是小的。
  名字都叫裁剪，剪的东西和取的方向都不一样——13.4 节有一张对照表专门管这件事。
...
机器人的动作是连续的数，没有"按蓝键的概率"这种东西，只有钟的高度（第 4 章 4.2 节：高度是密度，不是概率）。
...
### 代码走的是第二条路：ln 相减再 exp

采数据时，rsl_rl 把每个动作的 ln π 存了下来（第 11 章的映射块）；更新时，用现在的网络对同一个动作重算 ln π，两个相减，再过 exp。
为什么要绕一下，第 4 章 4.8 节说过：14 个关节的密度相乘，会小到下溢；取了 ln，乘法变加法，除法变减法。手算：

- ln 0.35207 = −1.0439，ln 0.38139 = −0.9639（浏览器里敲 `Math.log(0.35207)` 就能核对）；
- 相减：−0.9639 − (−1.0439) = 0.08；
...
先用 2 个关节的迷你版看一眼"加 ln 就是乘比率"：关节甲的比率 1.1，关节乙的 0.9。它们的 ln 之差是 ln 1.1 = 0.0953 和 ln 0.9 = −0.1054；
加起来 0.0953 + (−0.1054) = −0.0101，再过 exp：exp(−0.0101) = 0.99——正是 1.1 × 0.9 = 0.99。14 个关节也一样，只是加了 14 项。
...
真训练时，一份 24,576 条记录里会有多少条被封顶？实验第 3 节造了 10,000 条假记录来数一数：新旧 ln π 之差的标准差取 0.15（这个 0.15 是为了演示挑的，不是项目里的实测值）、Â 正负各半，结果 9.0% 落在平坦区。
...
一次 `backward()` 要同时管两张网络：actor（13.3 节的裁剪目标、熵）和 critic（第 10 章的价值估计）。
办法是把三样东西加成**一个**数，叫损失，然后一起往小里拧。这一节一项一项地装，每项都用小数字算一遍。

### 第一项：把裁剪目标翻个号，min 就变成了 max

torch 的优化器只会让损失变小（第 3 章 3.4 节的梯度下降）。$L^{\text{CLIP}}$ 要变大，就取个负号当损失——第 11 章 11.5 节 ⚠️ 里的那个负号。把 13.3 节的四格当成一小批，手算一次：
...
### 第二项：critic 的损失，外加一道语义不同的裁剪

第 10 章 10.5 节：critic 的损失是（估计 − 标签）²，标签是第 12 章 12.3 节的 returns = Â + V。
这里的 Â **不是** 13.3 节那个：returns 是采数据时就算好存下的，用的是**没归一化**的 Â（第 12 章 12.7 节的 ⚠️ 专门说过这件事）；归一化后的 Â 只给 actor 用。
critic 也有同一个毛病：这批记录只够它把自己往标签的方向挪一点点，一次迭代更新 20 次，它同样可能被同一批旧记录推得太远。
PPO 给它也装了一个限位，叫**价值裁剪**（value clipping）。

它是 13.3 节那一招搬到 critic 上，但**夹的东西和取的方向都换了**，得当一个新机制重新读一遍：

- **夹的东西换了**：13.3 节夹的是**比率**本身（0.8 到 1.2）；这里夹的是"现在的 V 离**采数据时那个 V** 挪了多远"（最多算挪 0.2）。
- **取的方向换了**：两边都是"往坏处想"（两个数里挑对自己不利的那个），可这里的"坏"反了个个儿——13.3 节的目标要**变大**，往坏处想就是取**小**的；这里是损失、要**变小**，往坏处想就是取**大**的。

手算一条记录：采数据时 critic 估 V = 0.2，标签是 0.5，所以"最多算挪 0.2"就是把 V 先夹进 0 到 0.4。更新几次之后，V 可能在三个位置：
...
一句话：挪过头的好处不算，往坏处跑照罚。实验第 4 节还量了三种情况的斜率：0、−0.3、−1.2。

![上图全景：横轴是更新后的V，灰虚线是不裁剪的(V−0.5)²、橙点线是先把V夹进0到0.4再算、蓝实线是两个取大的；V等于−0.1处取大的是0.36而裁剪版只有0.25；中图放大右半边：0.4到0.6之间蓝线是0.01的水平平台、斜率0，V等于0.55在平台上，V等于0.65又回到0.0225；下图对照表：比率裁剪夹比率取小的，价值裁剪夹V挪了多远取大的](../figures/ch13_value_clip.png)

**这样读图：** ① 先看三条线：灰虚线是不裁剪的 (V − 0.5)²，橙点线是"先把 V 夹进 0 ~ 0.4 再算"，蓝实线是两个里取大的——蓝线才是真正进损失的那个。
浅蓝竖条是 V 允许算进去的范围 0 ~ 0.4（旧 V 0.2 ± 0.2）。V 跑到 −0.1 时，灰的 0.36 比橙的 0.25 大，蓝线取的是灰的，照常把 critic 拉回来。
② 把右半边放大。0.4 到 0.6 之间蓝线是一段水平的橙色平台，高度恒是 0.01，斜率 0——V = 0.55 就落在上面，这条记录不再推 critic。
**可平台是有头的**：V 一过 0.6，(V − 0.5)² 又比 0.01 大了，取大的重新取到灰线，斜率回来（V = 0.65 时损失又是 0.0225，斜率 +0.3），critic 被拉回来。
这和 13.3 节的平坦区不一样：那里的平台一路向右不封口。③ 是两道裁剪的对照表，回头混的时候看这一张。

> ⚠️ **本章有三个 0.2，别串了。** ① 13.3 节的 0.2 是**比率**的倍数（夹进 0.8 ~ 1.2）；② 这里的 0.2 是**价值本身**的单位（V 最多算挪 0.2 分）——①② 读的是同一个设置 `clip_param=0.2`，意思却不同；
> ③ 13.5 节还会出现一个 0.2，那是**钟的中心挪了多远**（以 σ 为单位），和这个设置一点关系都没有，只是例子挑的数恰好也是 0.2。

> **停一下，自测**：同一条记录（旧 V = 0.2，标签 0.5），更新后 V 跑到了 0.7。不裁剪和裁剪版各是多少？取大的是哪个？这条记录还推不推 critic？
> <details><summary>答案</summary>
>
> 不裁剪 (0.7 − 0.5)² = 0.04；裁剪版还是把 V 当成 0.4，(0.4 − 0.5)² = 0.01。取大的是 **0.04**，用的是不裁剪版——所以它照常推，把 V 往 0.5 拉。
> 0.7 已经出了平坦区（0.4 ~ 0.6）：价值裁剪封的只是 0.4 到 0.6 这一段。
>
> </details>

### 第三项：熵，给"还愿意试"留一点分

第 11 章 11.6 节的熵奖励：要变大的是 c × H，进损失时取负号。项目的 c = 0.01。14 口钟一开始 σ 都是 1，熵是 14 × 1.41894 = 19.865，这一项就是 0.01 × 19.865 = 0.19865。
...
就拿第 1 格那条记录：ρ = 1.5、Â = 2；它的价值是上面 V = 0.55 那种情况；14 个 σ 都是 1：
...
**日志里的第一项，常常是个小负数**——这很正常，它的正负和大小都说明不了学得好不好。判断训练，要看 KL、回合长度、奖励各项和机器人实际的动作。真实的日志长什么样，第 14 章（训练回路全貌）对着冒烟训练的输出一行一行看。

<details>
<summary>进阶：为什么 Mean surrogate loss 一开始接近 0、之后变成小负数（现在可跳过）</summary>

训练日志的 `Mean surrogate loss:`（wandb 里叫 `Loss/surrogate`），就是 20 次更新的 `surrogate_loss` 取平均。
...
**这一段是从上面的算式推出来、再用构造的小例子算一遍的**：−0.044 是实验里编出来的数，不是从一次真实训练的日志里统计的。
...
这里的 0.2 是本章的第三个 0.2——"中心挪了多远"，和裁剪范围没关系（13.4 节的 ⚠️ 把三个 0.2 排过队）。
...
第 3 章的映射块预告过这条规则。每做一次更新**之前**，先量一次"现在的策略"和"采数据时的策略"之间的 KL，然后：
...
手算一串。学习率从 0.001 出发，连着量到六次 KL：

| 这次量到的 KL | 落在哪一段 | 怎么算 | 学习率 |
|---|---|---|---|
| 0.03 | 大于 0.02 | 0.001 ÷ 1.5 | 0.00066667 |
| 0.025 | 大于 0.02 | 再 ÷ 1.5 = 0.001 ÷ 2.25 | 0.00044444 |
| 0.012 | 中间（0.005 ~ 0.02） | 不动 | 0.00044444 |
| 0.004 | 小于 0.005 | × 1.5 | 0.00066667 |
| 0.003 | 小于 0.005 | 再 × 1.5 | 0.001 |
| 0.02 | **恰好**等于 0.02 | 不动（规则写的是"大于"） | 0.001 |

最后一列就是学习率本身，不是倍数：0.001 ÷ 1.5 = 0.00066667，再 ÷ 1.5 = 0.00044444（等于 0.001 ÷ 2.25）；
往回乘 1.5 两次，又回到 0.001。实验第 5 节按这六个 KL 把规则跑了一遍，打印的就是这一列。
...
> <details><summary>答案</summary>
>
> 0.021 大于 0.02：÷ 1.5，得 0.001 ÷ 1.5 = 0.00066667。0.02 不大于 0.02，也不小于 0.005：不动，还是 0.00066667。0.004 小于 0.005 且大于 0：× 1.5，回到 0.001。
>
> </details>
...
2. 比率：问卷的 0.5 份和 1.5 份；分量 1.4 和 0.6 修回 2.4（10 万局加权 2.4035）；进阶折叠里修回来的 6 和 −2；两口钟的高度 0.35207、0.38139 和它们的 ln（−1.0439、−0.9639），比率 1.0833，ln 之差 0.08，和 torch 的 `log_prob` 对照；自测的 2.8、1.2214、0.6703；1.02¹⁴ = 1.3195；画 `ch13_stale_data.png` 和 `ch13_ratio_density.png`。
...
4. 完整的损失：四格当一小批的 −0.3 和 0.3；价值裁剪的三种情况（0.01、0.0225、0.36）和斜率，以及平坦区只有 0.4 ~ 0.6 这一段（自测的 V = 0.7 和出界的 V = 0.65）；熵 19.865 和 0.19865；一条记录的总账 −2.58865；归一化的 Â 让 `surrogate_loss` 为 0、斜率不为 0，挪一点后变成 −0.044；画 `ch13_value_clip.png`。
...
- **三项损失**：loss = −$L^{\text{CLIP}}$ + 1.0 × 价值损失 − 0.01 × 熵。$L^{\text{CLIP}}$ 是**代理目标**：旧记录算出的替身，只在附近靠得住。取了负号，min 变 max。
  **价值裁剪**是同一个念头搬到 critic 上，可夹的东西和取的方向都换了：夹的是"V 离采数据时那个 V 挪了多远"（最多 0.2，单位是价值本身），损失要变小，所以两个里取**大**的。
  它的平坦区只有 0.4 ~ 0.6 这一段，V 再往外跑就又被拉回来。日志里的 surrogate loss 常是小负数，正常。
