...
四种情况逐格手算。每一格做两件事：先把 ρ 夹进 [0.8, 1.2]——小于 0.8 算 0.8，大于 1.2 算 1.2，中间的原样，这一步叫**裁剪**（clipping）；再把裁剪过的和没裁剪的两个结果比一比，取小的。
...
一大批记录里，有多少落在平坦区？实验第 3 节造了 10,000 条假记录：新旧 ln π 之差的标准差取 0.15、Â 正负各半，结果 9.0% 落在平坦区。
这个比例高，说明策略这一次迭代变得比较多，该去看看 13.5 节的 KL 和学习率；低也未必是好事，可能只是更新得很保守。rsl_rl 的训练日志不记这个数。
...
实验第 3 节用 torch 真的拧了 400 步：μ 停在 0.300，记录 1 的比率 1.742。
...
**第三项：熵。** 第 11 章 11.6 节的熵奖励：要变大的是 c × H，进损失时取负号。项目的 c = 0.01。14 口钟一开始 σ 都是 1，熵是 14 × 1.41894 = 19.865，这一项就是 0.01 × 19.865 = 0.199。
...
<details>
<summary>进阶：宽度也变时的公式；KL 为什么叫"散度"（现在可跳过）</summary>
...
条件里的"而且大于 0"，防的是新旧完全一样、KL 恰好是 0 的时候——比如每次迭代的第一次更新，旋钮还没动过（项目里这时的 KL 往往恰好是 0，本节最后的折叠块说为什么）——这时不该放大学习率。
...
旋钮是没动，可策略的输出还取决于输入归一化器（第 8 章）。它在采数据的 24 步里，每一步都在更新自己的平均和标准差（第 8 章 8.4 节）。
到了 `update()`，同一条观测归一化之后的数，已经和采数据时差了一点点，钟的中心也跟着差一点点。

实验第 5 节拿 rsl_rl 自己的网络类（带输入归一化）搭了一张 3 个输入的小网络来演示（数只是演示用的）：旋钮不动，只让归一化器再见一批整体偏了 0.3 的观测，然后重算。
...
同样的道理，项目里第一次更新的比率也不是恰好 1，只是非常接近 1。
...
第三，**省略号里跳过的**。开头第一个 `...` 里有一次 `self.actor(...)` 调用：用现在的 actor 把这一份观测过一遍，搭好现在的钟，下一行才能在钟上查 ln π。
...
正文还用到三处没贴代码的事实，也一并核对：KL 在 `rsl_rl/modules/distribution.py` 里算、14 个关节相加；每份的条数在 `rsl_rl/storage/rollout_storage.py` 里是"总条数 // 份数"；日志名 `Mean surrogate loss:` 和 `Loss/surrogate` 来自 `rsl_rl/utils/logger.py`。
...
4. 完整的损失：四格当一小批的 −0.3 和 0.3；价值裁剪的三种情况（0.01、0.0225、0.36）和斜率；熵 19.865 和 0.199；一条记录的总账 −2.58865；归一化的 Â 让 `surrogate_loss` 为 0、斜率不为 0，挪一点后变成 −0.044。
5. KL：0.02、0.005 和"翻四倍"；目标 0.01 对应的 0.1414 和 0.0378；14 个关节的 0.28；进阶折叠的 0.1284 和 0.0881；学习率的六步、上下限和自测；用 rsl_rl 的网络类演示归一化器带来的一点点 KL；画 `ch13_kl_shift.png`。
