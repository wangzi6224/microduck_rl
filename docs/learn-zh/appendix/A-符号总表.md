...
| $\sin\theta$、$\cos\theta$ | 正弦、余弦 | 长度 1 的箭头歪 θ 后的两个分量；从 0 补课见[附录 D](D-三角函数从0补课.md) | §2.4 |
| $\arccos x$ | 反余弦 | 由 cos 值反查角度：倾角 $=\arccos(-g_z)$ | §2.4 |
| rad | 弧度 | 角度的另一把尺子，$2\pi$ rad = 360°，1 rad ≈ 57.3°；代码只认它 | §2.4 |
